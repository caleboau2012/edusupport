...
{/if}  
<input type='text' name='{$fields.location_c.name}' 
id='{$fields.location_c.name}' size='30' 
maxlength='255' 
value='{$value}' title=''      >
</tr>
{/capture}
...
{if !isset($config.enable_autocomplete) || $config.enable_autocomplete==false}
<select name="{$fields.comm_channel_c.name}" 
id="{$fields.comm_channel_c.name}" 
title=''       
>
{if isset($fields.comm_channel_c.value) && $fields.comm_channel_c.value != ''}
{html_options options=$fields.comm_channel_c.options selected=$fields.comm_channel_c.value}
{else}
{html_options options=$fields.comm_channel_c.options selected=$fields.comm_channel_c.default}
{/if}
</select>
{else}
{assign var="field_options" value=$fields.comm_channel_c.options }
{capture name="field_val"}{$fields.comm_channel_c.value}{/capture}
{assign var="field_val" value=$smarty.capture.field_val}
{capture name="ac_key"}{$fields.comm_channel_c.name}{/capture}
{assign var="ac_key" value=$smarty.capture.ac_key}
<select style='display:none' name="{$fields.comm_channel_c.name}" 
id="{$fields.comm_channel_c.name}" 
title=''          
>
{if isset($fields.comm_channel_c.value) && $fields.comm_channel_c.value != ''}
...
<input
id="{$fields.comm_channel_c.name}-input"
name="{$fields.comm_channel_c.name}-input"
size="30"
value="{$field_val|lookup:$field_options}"
type="text" style="vertical-align: top;">
<span class="id-ff multiple">
<button type="button"><img src="{sugar_getimagepath file="id-ff-down.png"}" id="{$fields.comm_channel_c.name}-image"></button><button type="button"
id="btn-clear-{$fields.comm_channel_c.name}-input"
title="Clear"
onclick="SUGAR.clearRelateField(this.form, '{$fields.comm_channel_c.name}-input', '{$fields.comm_channel_c.name}');sync_{$fields.comm_channel_c.name}()"><img src="{sugar_getimagepath file="id-ff-clear.png"}"></button>
</span>
{literal}
<script>
SUGAR.AutoComplete.{/literal}{$ac_key}{literal} = [];
...
{literal}
(function (){
var selectElem = document.getElementById("{/literal}{$fields.comm_channel_c.name}{literal}");
if (typeof select_defaults =="undefined")
select_defaults = [];
select_defaults[selectElem.id] = {key:selectElem.value,text:''};
//get default
...
YUI().use("autocomplete", "autocomplete-filters", "autocomplete-highlighters", "node","node-event-simulate", function (Y) {
{/literal}
SUGAR.AutoComplete.{$ac_key}.inputNode = Y.one('#{$fields.comm_channel_c.name}-input');
SUGAR.AutoComplete.{$ac_key}.inputImage = Y.one('#{$fields.comm_channel_c.name}-image');
SUGAR.AutoComplete.{$ac_key}.inputHidden = Y.one('#{$fields.comm_channel_c.name}');
{literal}
function SyncToHidden(selectme){
var selectElem = document.getElementById("{/literal}{$fields.comm_channel_c.name}{literal}");
var doSimulateChange = false;
...
//global variable 
sync_{/literal}{$fields.comm_channel_c.name}{literal} = function(){
SyncToHidden();
}
function syncFromHiddenToWidget(){
var selectElem = document.getElementById("{/literal}{$fields.comm_channel_c.name}{literal}");
...
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.simulate('keyup');
for (i=0;i<selectElem.options.length;i++){
if (selectElem.options[i].value==selectElem.value && document.activeElement != document.getElementById('{/literal}{$fields.comm_channel_c.name}-input{literal}'))
SUGAR.AutoComplete.{/literal}{$ac_key}{literal}.inputNode.set('value',selectElem.options[i].innerHTML);
}
}
YAHOO.util.Event.onAvailable("{/literal}{$fields.comm_channel_c.name}{literal}", syncFromHiddenToWidget);
{/literal}
SUGAR.AutoComplete.{$ac_key}.minQLen = 0;
SUGAR.AutoComplete.{$ac_key}.queryDelay = 0;
SUGAR.AutoComplete.{$ac_key}.numOptions = {$field_options|@count};
...
{/if}  
<input type='text' name='{$fields.referred_by_c.name}' 
id='{$fields.referred_by_c.name}' size='30' 
maxlength='255' 
value='{$value}' title=''      >
</tr>
{/capture}
...
addToValidate('EditView', 'comm_channel_c', 'enum', false,'{/literal}{sugar_translate label='LBL_COMM_CHANNEL' module='Cases' for_js=true}{literal}' );
addToValidate('EditView', 'location_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_LOCATION' module='Cases' for_js=true}{literal}' );
addToValidate('EditView', 'referred_by_c', 'varchar', false,'{/literal}{sugar_translate label='LBL_REFERRED_BY' module='Cases' for_js=true}{literal}' );
...
addToValidateBinaryDependency('EditView', 'created_by_name', 'alpha', false,'{/literal}{sugar_translate label='ERR_SQS_NO_MATCH_FIELD' module='Cases' for_js=true}{literal}: {/literal}{sugar_translate label='LBL_CREATED' module='Cases' for_js=true}{literal}', 'created_by' );
