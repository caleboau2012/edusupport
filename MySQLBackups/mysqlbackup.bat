...
echo "Pass each db to mysqldump.exe and output an individual .sql file"
...
%mysqldumpexe% --user=%dbuser% --password=%dbpass% %database% --routines --log-error=%errorLogPath% > "%backupfldr%%database%.%backuptime%.sql"
...
%zipper% a -tzip "%backupfldr%%database%.%backuptime%.zip" "%backupfldr%*.sql"
